...
3. Run userspace with a chosen symbol file:
   ...
   - Explicit file: `sudo ./userspace s31_replication_1500Hz_5s_symbols.txt`
   - `make` shortcut: `make program SYMBOL_FILE=s31_replication_1500Hz_5s_symbols.txt`
4. Default userspace symbol file is `s31_replication_1500Hz_5s_symbols.txt` (5,000,000 symbols):
   - 1 microsecond PRU symbol timing
   - 1500 Hz requested OOK bit rate, 1499.250 Hz actual bit rate (`667` symbols per bit)
   - 677 complete repeats of the 11-bit replication message
   - OFF guard/pad symbols fill the rest of the 5 second window
...
6. Legacy `s31_pw_1p00ms_symbols.txt` / `0p75ms` / `0p50ms` / `0p25ms` files were for the earlier 0.25 ms PRU timing, `s31_pw50us_*` files were for the 0.05 ms PRU timing, and `s31_pw10us_*` files were for the 0.01 ms PRU timing. Do not mix any of those families with the current 0.001 ms firmware.
...
3. Start one transmission:
   - `sleep 1 && SYMLEN=$(wc -c < userspace/s31_replication_1500Hz_5s_symbols.txt) && echo -n "$SYMLEN" > /dev/rpmsg_pru31 && cat /dev/rpmsg_pru31`
