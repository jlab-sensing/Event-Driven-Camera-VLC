...
3. Run userspace with a chosen symbol file:
   ...
   - Explicit file: `sudo ./userspace s31_pw1us_0p005ms_symbols.txt`
   - `make` shortcut: `make program SYMBOL_FILE=s31_pw1us_0p005ms_symbols.txt`
...
3. Start one transmission (example uses the 0.005 ms pulse file):
   - `sleep 1 && SYMLEN=$(wc -c < userspace/s31_pw1us_0p005ms_symbols.txt) && echo -n "$SYMLEN" > /dev/rpmsg_pru31 && cat /dev/rpmsg_pru31`
