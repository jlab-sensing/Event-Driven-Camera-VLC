...
3. Run userspace with a chosen symbol file:
   ...
   - Explicit file: `sudo ./userspace s31_pw10us_0p04ms_symbols.txt`
   - `make` shortcut: `make program SYMBOL_FILE=s31_pw10us_0p04ms_symbols.txt`
4. Default userspace symbol file is `s31_lux_sweep_1000Hz_symbols.txt` (14,000 symbols):
   - 2,000 OFF symbols (`0`)
   - 10,000 symbols alternating `4`/`0`
   - 2,000 OFF symbols (`0`)
5. Current follow-up pulse-width sweep files (all 1,400,000 symbols, built for 0.01 ms PRU symbol timing):
   - `s31_pw10us_0p05ms_symbols.txt` (control run at the previous best-known working width)
   - `s31_pw10us_0p04ms_symbols.txt`
   - `s31_pw10us_0p03ms_symbols.txt`
   - `s31_pw10us_0p02ms_symbols.txt`
   - `s31_pw10us_0p01ms_symbols.txt`
6. Legacy `s31_pw_1p00ms_symbols.txt` / `0p75ms` / `0p50ms` / `0p25ms` files were for the earlier 0.25 ms PRU timing, and `s31_pw50us_*` files were for the 0.05 ms PRU timing. Do not mix either family with the current 0.01 ms firmware.
...
2. Build/load PRU firmware (symbol timing currently 0.01 ms): `make clean && make program`
3. Start one transmission (example uses the 0.04 ms pulse file):
   - `sleep 1 && SYMLEN=$(wc -c < userspace/s31_pw10us_0p04ms_symbols.txt) && echo -n "$SYMLEN" > /dev/rpmsg_pru31 && cat /dev/rpmsg_pru31`
